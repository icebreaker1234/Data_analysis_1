...
# ------------------------------
# 1) Generate synthetic access log
# ------------------------------
...
event_types = ["LOGIN_SUCCESS", "LOGIN_FAIL", "FILE_READ", "FILE_WRITE", "DB_QUERY", "PROCESS_START"]

data = []

for _ in range(n_sessions):
    user = np.random.choice(users)
    events = np.random.randint(5, 15)  # events per session
    for _ in range(events):
        data.append({
            "user_id": user,
            "event_type": np.random.choice(event_types),
            "bytes_transferred": np.random.randint(100, 5000),
            "access_result": np.random.choice(["SUCCESS", "FAIL"], p=[0.9, 0.1])
        })

df = pd.DataFrame(data)
print("Sample synthetic access log:")
print(df.head())

# ------------------------------
# 2) Feature engineering (simple)
# ------------------------------
# Aggregate per user session (or just per user here for simplicity)

features = df.groupby("user_id").agg(
    total_events=("event_type", "count"),
    file_reads=("event_type", lambda x: sum(x=="FILE_READ")),
    file_writes=("event_type", lambda x: sum(x=="FILE_WRITE")),
    db_queries=("event_type", lambda x: sum(x=="DB_QUERY")),
    process_starts=("event_type", lambda x: sum(x=="PROCESS_START")),
    login_fails=("access_result", lambda x: sum(x=="FAIL")),
...
model.fit(X)
features['anomaly'] = model.predict(X)  # -1 = anomaly, 1 = normal
features['anomaly'] = features['anomaly'].apply(lambda x: 1 if x==-1 else 0)

print("\nAnomaly detection results:")
print(features[['user_id', 'anomaly']])
